...
{ 'construct'( context )
    { context.shared_scratchpad.animate = true;
      ...
      this.newest_shapes = { "donut"       : new Torus( 15, 15, 0.5 ),
                             ...
                             "gem2"        :     Torus             .prototype.auto_flat_shaded_version( 20, 20, 0.5 ) };
...
{ 'simulate'(time)
    { if   ( this.bodies.length > 20 )       this.bodies = this.bodies.splice( 0, 20 );                 // Max of 20 bodies
      ...
      { var b_inv = inverse( mult( b.location_matrix, scale( b.scale ) ) );               // Cache b's final transform
        ...
        b.material = new Material( Color( 1,1,1,1 ), .1, .5, .5, 40 );                      // Default color: white
        ...
        for( let c of this.bodies )                                      // Collision process starts here
          ...
          { b.material = new Material( Color( 1,0,0,1 ), .1, .5, .5, 40 ); // If we get here, we collided, so turn red
